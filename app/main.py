...
from models.sitemap import Sitemap as SitemapModel
import os
...
class Sitemap:
    ...
    def __init__(self, base_url):
        self._urls = []
        self._sitename = urlparse(base_url).netloc  # Obtaining the website name

    def add_url(self, domain: str, url: str, date: datetime):
        self._urls.append({"site": domain, "loc": url, "lastmod": date})
    ...
    def create_sitemap(self):
        ...
        if multi_file:
            ...
            for i in range(files_num):
                ...
                self._add_sitemap_in_database(
                    self._urls[
                        i * self._MAX_URL_PER_FILE : (i + 1) * self._MAX_URL_PER_FILE
                    ]
                )
                self._create_sitemap_file(
                    filename,
                    self._urls[
                        i * self._MAX_URL_PER_FILE : (i + 1) * self._MAX_URL_PER_FILE
                    ],
            ...
            self._add_sitemap_in_database(self._urls)
            self._create_sitemap_file(
                f"expamles/sitemap_{self._sitename}.xml", self._urls
            )

    def _create_index_sitemap_file(self, sitemap_file_names):
        with open("sitemap_index.xml", "w") as f:
            f.write(f"{self._xml_header}\n")
    ...
    def _add_sitemap_in_database(self, urls):
        with Session() as session:
            session.bulk_insert_mappings(SitemapModel, urls)
            session.commit()
    ...
    def _create_sitemap_file(self, filename: str, urls):
        with open(filename, "w") as f:
            ...
            for url in urls:
                ...
                f.write(
                    f"      <lastmod> {url['lastmod'].strftime('%Y-%m-%dT%H:%M:%S+00:00')} </lastmod>\n"
                )
                f.write("   </url>\n")
...
class Crawler(threading.Thread):
    ...
    def _check_link_str(self, link_str, domain_link, link_obj):
    ...
    def run(self):
        ...
        while True:
            ...
            # Add url to sitemap
            self.sitemap.add_url(
                domain=urlparse(response_url_string).netloc,
                url=self._convert_html_special_chars(response_url_string),
                date=date,
            )
            ...
            links: list = self.link_regex.findall(response_data)
            for link_str in links:
                link_str = link_str.decode("utf-8", errors="ignore")

                if link_str.startswith("/"):
                    link_str = (
                        response_url_obj.scheme + "://" + response_url_obj[1] + link_str
                    )
                elif link_str.startswith("#"):
                    link_str = (
                        response_url_obj.scheme
                        + "://"
                        + response_url_obj[1]
                        + response_url_obj[2]
                        + link_str
                ...
                if not self._check_link_str(link_str, domain_link, link_obj):
                    continue
...
class CrawlingManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._sitemap = Sitemap(base_url)
    ...
    def run(self):
        ...
        for i in range(self._num_workers):
            crawler = Crawler(
                self._base_url,
                self._crawling_queue,
                self._crawled_urls,
                self._excluded_urls,
                self._sitemap,
            )
            crawler.name = f"Crawler-{i}"
            crawler.start()
        ...
        logging.info("Sitemap files successful created in database and same directory")
...
def main():
    ...
    mng = CrawlingManager(
        base_url=os.environ.get("SITE"), num_workers=int(os.environ.get("NUM_WORKERS"))
    )
    mng.run()
